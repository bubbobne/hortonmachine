...
public class ModelsEngine {
	/**
	 * @deprecated this should not be used. Use the {@link FlowNode} concept
	 *             instead.
	 */
	 ...
	 *
	 * @param colRow        the array containing the column and row of the current
	 *                      pixel. It will be modified here to represent the next
	 *                      downstream pixel.
	 ...
	 * @param param
	 */
	public static void goUpStreamOnNetFixed(int[] colRow, RandomIter flowIterator, RandomIter netnumIterator,
			int[] param) {
		...
		for (int k = 1; k <= 8; k++) {
			if (flowIterator.getSampleDouble(colRow[0] + dirIn[k][1], colRow[1] + dirIn[k][0], 0) == dirIn[k][2]) {
				count++;
				if (netnumIterator.getSampleDouble(colRow[0] + dirIn[k][1], colRow[1] + dirIn[k][0],
						0) == netnumIterator.getSampleDouble(colRow[0], colRow[1], 0)) {
					kk = k;
	...
	public static SimpleFeatureCollection net2ShapeOnly(RenderedImage flowImage, WritableRaster netNumImage,
			GridGeometry2D gridGeometry, List<Integer> nstream, IHMProgressMonitor pm)
					...
					if (!isNovalue(m1RandomIter.getSampleDouble(x, y, 0))) {
						...
						if (netNumRandomIter.getSampleDouble(x, y, 0) == num) {
							...
							if (sourcesNet(m1RandomIter, flow, num, netNumRandomIter)) {
								flow_p[0] = flow[0];
								flow_p[1] = flow[1];
								double[] worldPosition = gridGeometry
										.gridToWorld(new GridCoordinates2D(flow[0], flow[1])).getCoordinate();
	 ...
	 * @throws TransformException
	 */
	public static List<MultiLineString> net2ShapeGeometries(WritableRandomIter flowIter, RandomIter netNumIter,
			int[] nstream, GridGeometry2D gridGeometry, IHMProgressMonitor pm) throws IOException, TransformException {
		...
		for (int num = 1; num <= nstream[0]; num++) {
			...
			newGeometryVectorLine.add(newfactory.createMultiLineString(
	...
	public static boolean sourcesNet(RandomIter flowIterator, int[] colRow, int num, RandomIter netNum) {
		int[][] dir = { { 0, 0, 0 }, { 1, 0, 5 }, { 1, -1, 6 }, { 0, -1, 7 }, { -1, -1, 8 }, { -1, 0, 1 }, { -1, 1, 2 },
				{ 0, 1, 3 }, { 1, 1, 4 } };
	...
	public static double split2realvectors(double[] U, double[] T, SplitVectors theSplit, int binNum, int num_max,
		...
		if (binNum <= 1) {
			...
			while (count < U.length) {
				...
				count++;
				if (head > num_max)
					throw new ModelsIllegalargumentException("The number of bin exceeds the maximum number allowed.",
							"MODEL", pm);
			}
		...
		if (head < 1) {
			...
			/*
			 * now a list of the values inside the bins are put into the matrixes, therefore
			 * we need as many rows as bins and a column number high enough to hold the
			 * major number of values hold inside a bin.
			 */
	 ...
	 * @param pointsFC     optional feature collection of points in which to split
	 *                     the net.
	 * @param pm           the monitor.
	 * @return the raster of netnumbering.
	 * @throws Exception
	 */
	public static HMRaster netNumbering(HMRaster flowR, HMRaster netR, HMRaster tcaR, List<Geometry> points,
			List<NetLink> netLinksList, IHMProgressMonitor pm) throws Exception {
	 ...
	 * @throws IOException
	 */
	public static HMRaster extractSubbasins(HMRaster flowRaster, HMRaster netRaster, HMRaster netNumberRaster,
			IHMProgressMonitor pm) throws IOException {
	...
	/**
	 * Marks a map on the hillslope with the values on the channel of an attribute
	 * map.
	 *
	 * @param flowRaster      map of flow direction with the network cells all
	 *                        marked as {@link FlowNode#NETVALUE}. This is very
	 *                        important!
	 ...
	 * @param pm              monitor.
	 */
	public static void markHillSlopeWithLinkValue(HMRaster flowRaster, HMRaster attributeRaster, HMRaster markedRaster,
			IHMProgressMonitor pm) {
		RegionMap regionMap = flowRaster.getRegionMap();
		int cols = regionMap.getCols();
		int rows = regionMap.getRows();
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				...
				if (flowNode.isValid() && flowNode.isSource()) {
					/*
					 * run down to the net to find the attribute map content on the net
					 */
					...
					if (!attributeRaster.isNovalue(attributeValue)) {
						...
						RasterCellInfo cellInfo = new RasterCellInfo(runningCol, runningRow, flowRaster,
								attributeRaster, markedRaster);
	...
	/**
	 * Returns the flow direction value for a given point as indexes (i, j) of the
	 * dir matrix.
	 *
	 * @param i
	 * @param j
	...
	public static double widthInterpolate(double[][] data, double x, int nx, int ny) {
		...
		if (x > data[(rows - 1)][nx] || x < 0) {
			throw new RuntimeException(MessageFormat.format(
					"Error in the interpolation algorithm: entering with x = {0} (min = 0.0 max = {1}", x,
					data[(rows - 1)][nx]));
		}
	...
	/**
	 * Calculates the sum of the values of a specified quantity from every point to
	 * the outlet.
	 *
	 * <p>
	 ...
	 * @param demWR            the elevation map.
	 * @param dx               the resolution of the elevation map.
	 ...
	 * @param tmpWR            the output shadow map.
	 * @param demWR            the elevation map.
	 * @param res              the resolution of the elevation map.
	 * @param normalSunVector
	 * @param inverseSunVector
	 * @return
	 */
	 ...
	 * @throws Exception
	 */
	public static boolean verifyDoubleStation(double[] xStation, double[] yStation, double[] zStation,
			double[] hStation, double xTmp, double yTmp, double zTmp, double hTmp, int i, boolean doMean)
			throws Exception {
	 ...
	 * matrix. It's necessary to specify the initial and final index of the coluns
	 * to sum.
	 * </p>
	 *
	 *
	...
	/**
	 * Calculates the distance of every pixel of the basin from the outlet (in
	 * meter), calculated along the drainage directions
	 *
	 * @param flowIter          the flow map.
	 * @param pitIter           the pit map (if available distance is calculated in
	 *                          3d).
	 * @param distanceToOutIter the resulting outlet distance map.
	 * @param region            the region parameters.
	 * @param pm                the monitor.
	 */
	public static void topologicalOutletdistance(RandomIter flowIter, RandomIter pitIter,
			WritableRandomIter distanceToOutIter, RegionMap region, IHMProgressMonitor pm) {
	...
	/**
	 * Calculates the distance of every pixel of the basin from the outlet (in map
	 * units), calculated along the drainage directions
	 *
	 * @param flowIter          the flow map.
	 * @param distanceToOutIter the resulting outlet distance map.
	 * @param region            the region parameters.
	 ...
	 *
	 * <p>
	 * It evaluates a multiple of the divisor (which is the nearest number to
	 * valueToApproximate).
	 * </p>
	 *
	 * @param valueToApproximate value to approximate
	 * @param divisor            the unit value, it's the divisor number.
	 * @return the largest value less than or equal to valueToApproximate and
	 *         divisible to divisor.
	 */
